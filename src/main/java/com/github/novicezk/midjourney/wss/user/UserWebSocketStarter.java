...
public class UserWebSocketStarter extends WebSocketAdapter implements WebSocketStarter {
	...
	public UserWebSocketStarter(String wssServer, String resumeWss, DiscordAccount account, UserMessageListener userMessageListener,
			AccountStoreService accountStoreService, ProxyProperties.ProxyConfig proxyConfig) {
	...
	public synchronized void start() throws Exception {
		...
		String gatewayUrl = getGatewayServer() + "/?encoding=json&v=9&compress=zlib-stream";
	...
	private String getGatewayServer() {
		if (CharSequenceUtil.isNotBlank(this.resumeGatewayUrl)) {
			return CharSequenceUtil.isBlank(this.resumeWss) ? this.resumeGatewayUrl : this.resumeWss;
		}
	...
	@Override
	public void stop() {
		log.debug("[wss-{}] Trigger stop wss.", this.account.getDisplay());
		sendClose(5240, "trigger stop wss");
		clearAllStates();
	}
	...
	private void tryReconnect() {
		...
		try {
			...
			log.warn("[wss-{}] Reconnect fail: {}, Waiting try new connection...", this.account.getDisplay(), e.getMessage());
	...
	private void disableAccount() {
		...
		ThreadUtil.execute(() -> {
			...
			this.accountStoreService.save(this.account);
		});
	}
	...
	private void handleDispatch(DataObject raw) {
		...
		if ("READY".equals(t)) {
			...
			this.account.setSessionId(this.sessionId);
			DataObject user = content.getObject("user");
			this.account.setName(user.getString("username"));
			this.account.setUserId(user.getString("id"));
			this.account.setEmail(user.getString("email"));
