...
public class ErrorMessageHandler extends MessageHandler {
	...
	public void handle(DiscordInstance instance, MessageType messageType, DataObject message) {
		...
		if (color == 16239475) {
			...
			if (task != null) {
				task.fail(convertReason("[" + title + "] " + description));
				task.awake();
			}
		} else {
	...
	private String convertReason(String error) {
		for (ProxyProperties.ErrorDesc errorDesc : this.properties.getErrorDesc()) {
			if (error.contains(errorDesc.getEn())) {
				return errorDesc.getZh();
			}
		}
		return error;
	}
