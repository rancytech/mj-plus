	...
	public void checkTasks() {
		this.discordLoadBalancer.getAliveInstances().forEach(instance -> {
			...
			List<Task> tasks = instance.getRunningTasks().stream()
					...
					.toList();
			for (Task task : tasks) {
				if (Set.of(TaskStatus.FAILURE, TaskStatus.SUCCESS, TaskStatus.CANCEL).contains(task.getStatus())) {
					log.warn("task status is failure/success/cancel but is in the queue, end it. id: {}", task.getId());
				} else {
